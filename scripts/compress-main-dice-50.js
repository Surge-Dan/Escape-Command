#!/usr/bin/env node
// 把首页 3D 骰子（从 packageDice 大图复制来的）压缩到 50%
const fs = require('fs');
...
const tempDir = path.join(os.tmpdir(), 'escape_dice_main_50');
...
targets.forEach((name, i) => {
  ...
  try {
    ...
    origTotal += origSize;
    newTotal += newSize;
    console.log('  ' + name.padEnd(20) + (origSize/1024).toFixed(1).padStart(8) + ' -> ' + (newSize/1024).toFixed(1).padStart(7) + ' KB  (-' + savings + '%)');
  } catch (e) {
