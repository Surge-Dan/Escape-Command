...
const tempDir = path.join(os.tmpdir(), 'escape_dice40');
...
targets.forEach((name, i) => {
  ...
  try {
    ...
    origTotal += origSize;
    newTotal += newSize;
    console.log(`  ${name.padEnd(30)} ${(origSize/1024).toFixed(1).padStart(8)} -> ${(newSize/1024).toFixed(1).padStart(8)} KB  (-${savings}%)`);
  } catch (e) {
