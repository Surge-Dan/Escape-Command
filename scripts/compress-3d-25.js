...
const tempDir = path.join(os.tmpdir(), 'escape_dice25');
...
targets.forEach((name, i) => {
  ...
  try {
    ...
    origTotal += origSize;
    newTotal += newSize;
    console.log('  ' + name.padEnd(20) + (origSize/1024).toFixed(1).padStart(8) + ' -> ' + (newSize/1024).toFixed(1).padStart(7) + ' KB  (-' + savings + '%)  scale=0.25');
  } catch (e) {
